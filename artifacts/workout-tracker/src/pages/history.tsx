...
import { useSessionHistory, useScheduledSessions } from "@/hooks/use-history";
...
function groupByMonth(sessions: SessionSummary[]): MonthGroup[] {
  ...
  for (const session of sessions) {
    const reference = session.completedAt ?? session.startedAt;
...
export default function HistoryPage() {
  ...
  const { remove, activateScheduled, cancel } = useSessionMutations();
  ...
  return (
    <Layout title="History">
      <div className="p-4 pb-20">
        <Tabs value={tab} onValueChange={(v) => setTab(v as "all" | "scheduled")}>
          ...
          <TabsContent value="scheduled" className="mt-0">
            {scheduledLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                ...
                <h3 className="text-xl font-bold mb-2">Nothing scheduled</h3>
                <p className="text-muted-foreground">
                  Schedule a workout from a plan and it will show up here.
                </p>
              </div>
            ) : (
              <div className="grid gap-3">
                {scheduled.map((session) => {
                  ...
                  const isPast = date ? isBefore(startOfDay(date), startOfDay(new Date())) : false;
                  const isReady = date ? !isBefore(startOfDay(new Date()), startOfDay(date)) : false;

                  return (
                    <div
                      key={session.id}
                      className={`bg-card rounded-2xl card-shadow p-4 relative overflow-hidden ${isPast ? "opacity-60" : ""}`}
                    >
      ...
      <AlertDialog open={!!cancelScheduledTarget} onOpenChange={(open) => !open && setCancelScheduledTarget(null)}>
        <AlertDialogContent className="max-w-sm mx-4">
          <AlertDialogHeader>
